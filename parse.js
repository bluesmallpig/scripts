...
// @version      1.1.8.8
// @license      AGPL-3.0
// @description  无需跳转新网址，打开官网直接看，蓝光 4k 无广告 随机去水印。支持：腾讯，爱奇艺，优酷，哔哩哔哩，咪咕，乐视，搜狐，芒果，西瓜，PPTV，1905电影网，华数。支持解析失败自动切换推荐解析源。适配各种浏览器。
    ...
    if (window === top) {
        ...
        */
        settings.NoAD解析 = {    // TODO by 17kyun.com/api.php?url=    // TODO by tv.hzwdd.cn
                    ...
                    "PPJ解析": "https://jx.ppjbk.cn/?url=",    // TODO 腾讯 (芒果) (B站)
                    "PPJ弹幕解析2": "https://bf.ppjbk.cn/bf/?url=",    // TODO 腾讯 (芒果)
                    ...
                    "虾米解析": "https://jx.xmflv.com/?url=",    // TODO (土豆) (咪咕) 搜狐 (芒果)
                    "九八看解析": "https://jx.ap2p.cn/?url=",    // (Parwix解析: "https://jx.parwix.com:4433/player/analysis.php?v=" || "https://vip.parwix.com:4433/player/?url=" || )    // TODO (西瓜) (B站) (搜狐) (芒果)
    ...
    if (window === top) {
        ...
        if (!detectMobile()) {
            if (location.host.indexOf("v.qq.com") !== -1) {
                readyPlayerBox("已进入腾讯视频", ["#mask_layer", ".mod_vip_popup", "#mask_layer"], [settings.NoAD解析["PPJ蓝光解析"], settings.NoAD解析["OK解析"], settings.NoAD解析["天翼解析"], settings.NoAD解析["夜幕解析"]], "div#mod_player", null);
            } else if (location.host.indexOf("iqiyi.com") !== -1) {
                readyPlayerBox("已进入爱奇艺", ["#playerPopup", "div[class^=qy-header-login-pop]"], [settings.NoAD解析["PPJ蓝光解析"], settings.NoAD解析["OK解析"]], "iqpdiv.iqp-player[data-player-hook$=er],m-player", null);
            } else if (location.host.indexOf("youku.com") !== -1) {
                readyPlayerBox("已进入优酷视频", ["#iframaWrapper"], [settings.NoAD解析["PPJ蓝光解析"]], "div#player", null);
                ...
                doElement("svg[aria-hidden=true]", () => readyPlayerBox("已进入哔哩哔哩", null, [settings.NoAD解析["夜幕解析"], settings.NoAD解析["PPJ蓝光解析"]], "div.bpx-player-video-area,div.mask-container", null));    // TODO || document.getElementById("bilibiliPlayer") || document.getElementById("live-player-ctnr")
                ...
                readyPlayerBox("已进入芒果TV", null, [settings.NoAD解析["PPJ解析"], settings.NoAD解析["PPJ蓝光解析"], settings.NoAD解析["OK解析"], settings.NoAD解析["虾米解析"], settings.NoAD解析["夜幕解析"]], "#mgtv-player-wrap", null);
            ...
            if (location.host.indexOf("v.qq.com") !== -1) {
                ...
                readyPlayerBox("已进入爱奇艺", ["div.m-iqyGuide-layer", "a[down-app-android-url]", "[name=m-extendBar]", "[class*=ChannelHomeBanner]", "section.m-hotWords-bottom"], [settings.NoAD解析["PPJ蓝光解析"], settings.NoAD解析["OK解析"]], "section.m-video-player", null);
            } else if (location.host.indexOf("youku.com") !== -1) {
                readyPlayerBox("已进入优酷视频", ["#iframaWrapper", ".ad-banner-wrapper", ".h5-detail-guide,.h5-detail-vip-guide", ".brief-btm"], [], "#player", null);
            } else if (location.host.indexOf("bilibili.com") !== -1) {
                readyPlayerBox("已进入哔哩哔哩", ["div.fe-ui-open-app-btn,div.recom-wrapper,open-app-btn", "[class*=openapp]"], [settings.NoAD解析["PPJ蓝光解析"], settings.NoAD解析["夜幕解析"]], "div#app.main-container div.player-wrapper", null, href => href.replace("m.bilibili.com", "www.bilibili.com"));
            } else if (location.host.indexOf("miguvideo.com") !== -1) {
                ...
                readyPlayerBox("已进入PPTV", ["[data-darkreader-inline-bgimage][data-darkreader-inline-bgcolor]", "div[class^=pp-m-diversion]", "section#ppmob-detail-picswiper", "section.layout.layout_ads", "div.foot_app", "div[modulename=导流位]", "a[class*=user]", "div.mod_video_info div.video_func"], [settings.NoAD解析["PPJ蓝光解析"]], "section.pp-details-video", null, href => href.replace("m.pptv.com", "v.pptv.com"));
            } else if (location.host.indexOf("vip.1905.com") !== -1) {
                (movie_info => {
